...
from rar.utils.logger import setup_logger
from rar.utils.train_utils import (
    ...
    auto_resume, save_checkpoint, create_wm_model,
...
def print_attrs(obj, depth=0, max_depth=3):
    indent = "  " * depth
    if depth > max_depth:
    ...
    if hasattr(obj, "__dict__"):
        for k, v in vars(obj).items():
            ...
            print_attrs(v, depth+1, max_depth)
...
def main():
    ...
    if accelerator.is_main_process:
        ...
        if tracker == "tensorboard":
            ...
            accelerator.print("Starting Wandb setup.")
            accelerator.init_trackers(
                project_name=config.experiment.project,
                config=OmegaConf.to_container(config, resolve=True, throw_on_missing=True),
                init_kwargs={"wandb": {"entity": "avseal", "name": f"{config.experiment.name}_{config.optimizer.params.learning_rate}"}},
            )
    ...
    # For distillation of transformer, we use  maskgit-vq tokenizer
    tokenizer = create_pretrained_tokenizer(config)
    if tokenizer is not None:
        tokenizer.to(accelerator.device)

    model, ema_model, ref_model, loss_module = create_model_and_loss_module(
    ...
    wm_model, wm_msg = create_wm_model(config)
    if wm_model is not None:
        wm_model.to(accelerator.device)
        wm_msg = wm_msg.to(accelerator.device)
    ...
    if config.dataset.params.get("pretokenization", ""):
        logger.info("Preparing model, optimizer, scheduler and dataloaders")
        if wm_model is not None:
            model, optimizer, wm_model, lr_scheduler, train_dataloader = accelerator.prepare(
                model, optimizer, wm_model, lr_scheduler, train_dataloader
            )
        else:
        ...
        logger.info("Preparing model, optimizer and scheduler")
    ...
    logger.info(f"  Decoder Finetune = {os.getenv('FT_DECODER', None) is not None}")
    logger.info(f"  Distillation mode = {ref_model is not None}")
    global_step = 0
    first_epoch = 0
    ...
    for current_epoch in range(first_epoch, num_train_epochs):
        accelerator.print(f"{datetime.now().strftime('%Y-%m-%d %H:%M:%S')}: Epoch {current_epoch}/{num_train_epochs-1} started.")
        global_step = train_one_epoch_generator(config, logger, accelerator,
                            model, ema_model, ref_model, loss_module, 
                            optimizer,
                            lr_scheduler,
                            train_dataloader,
                            tokenizer,
                            ...
                            wm_msg=wm_msg)
        if accelerator.is_main_process:
            accelerator.print(f"Global step at the end of epoch {current_epoch}: {global_step}")
