...
from deps.efficientvit.diffusioncore.models.uvit_sampler.dpm_solver_pp import DPM_Solver, NoiseScheduleVP
from deps.efficientvit.models.utils.network import get_device, get_submodule_weights
...
class UViT(nn.Module):
    ...
    def forward(self, x, y, generator: Optional[torch.Generator] = None, forward_reference=None, reference_model_weight: float = 0.0):
        info = {}
        ...
        if reference_model_weight > 0.0:
            with torch.no_grad():
                eps_reference = forward_reference(xn, n, y)
            loss_reference = (eps_pred - eps_reference).square().mean()
            info["loss_dict"]["loss_reference"] = loss_reference
            loss += reference_model_weight * loss_reference
        return loss, info

    @torch.no_grad()
    def generate(self, inputs, null_inputs, scale: float = 1.5, generator: Optional[torch.Generator] = None, with_history: bool = False, intermediate_samples=None):
        ...
        if self.cfg.eval_scheduler == "DPM_Solver":
            ...
            if with_history:
                samples, history = dpm_solver.sample(samples, steps=self.cfg.num_inference_steps, eps=1.0 / N, T=1.0, with_history=True, intermediate_samples=intermediate_samples)
