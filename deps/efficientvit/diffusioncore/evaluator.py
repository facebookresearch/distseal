...
from deps.efficientvit.apps.metrics.inception_score.inception_score import InceptionScoreStats, InceptionScoreStatsConfig
...
from deps.efficientvit.diffusioncore.data_provider.sample_class import SampleClassDataProvider, SampleClassDataProviderConfig
from deps.efficientvit.diffusioncore.models.dit import DiT, DiTConfig
from deps.efficientvit.diffusioncore.models.uvit import UViT, UViTConfig
...
class EvaluatorConfig:
    ...
    # Watermark
    watermark_ckpt_path: Optional[str] = None
    watermark_scaling_w: Optional[float] = None
    watermark_msg_path: Optional[str] = None
...
class Evaluator:
    def __init__(self, cfg: EvaluatorConfig):
        ...
        # Watermark detection.
        if cfg.watermark_ckpt_path is not None:
            assert cfg.watermark_scaling_w is not None
            assert cfg.watermark_msg_path is not None
            watermarker = setup_model_from_checkpoint(cfg.watermark_ckpt_path)
            watermarker.blender.scaling_w = cfg.watermark_scaling_w
            watermarker = watermarker.to(device)
            watermarker = watermarker.eval()
            self.msg = np.load(cfg.watermark_msg_path)
    ...
    def evaluate(self, step: int, network: Optional[nn.Module] = None, f_log=sys.stdout) -> dict[str, Any]:
        ...
        with tqdm(
            ...
            for _, (inputs, inputs_null) in enumerate(dataloader):
                ...
                if self.cfg.model in ["dit", "uvit"]:
                    ...
                    if self.watermarker is not None:
                        watermark_logits = self.watermarker.detector(image_samples_uint8.float() / 255.)[:, 1:]
                        pred_bits = (watermark_logits > 0).float().cpu().numpy()
                        bit_acc = (pred_bits == self.msg).astype(np.float32).mean(1)
        ...
        if self.watermarker is not None:
            valid_info_dict["mean_bit_acc"] = bit_acc_stats.get_mean_bit_acc()
