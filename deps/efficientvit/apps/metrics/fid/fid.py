...
from deps.efficientvit.apps.metrics.fid.inception import InceptionV3
from deps.efficientvit.apps.utils.dist import get_dist_local_rank, is_dist_initialized, is_master, sync_tensor
...
class WatermarkStats:
    ...
    @torch.no_grad
    def add_data(self, bit_acc):
        self.num_samples += bit_acc.shape[0]
        self.sum_bit_acc += bit_acc.sum()

    def get_mean_bit_acc(self):
        num_samples, sum_bit_acc = self.num_samples, self.sum_bit_acc
        if is_dist_initialized():
            num_samples = sync_tensor(torch.tensor(num_samples).cuda(), reduce="sum").cpu().numpy()
            sum_bit_acc = sync_tensor(torch.tensor(sum_bit_acc).cuda(), reduce="sum").cpu().numpy()
            if not is_master():
                return 0
        mean_bit_acc = sum_bit_acc / num_samples
        return mean_bit_acc
