...
from deps.efficientvit.models.utils import is_parallel
...
class EMA:
    ...
    def load_state_dict(self, state_dict: dict[float, dict[str, torch.Tensor]], strict: bool = True) -> None:
        for decay in state_dict:
            if decay == self.decay:
                self.shadows.load_state_dict(state_dict[decay], strict=strict)
