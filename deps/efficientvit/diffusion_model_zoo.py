...
from deps.efficientvit.diffusioncore.models.dit import dc_ae_dit_xl_in_512px
from deps.efficientvit.diffusioncore.models.uvit import (
...
def create_dc_ae_diffusion_model_cfg(name: str, pretrained_path: Optional[str] = None) -> EvaluatorConfig:
    if name not in REGISTERED_DCAE_DIFFUSION_MODEL:
        raise ValueError(f"Unknown diffusion model name: {name}. Registered models are: {list(REGISTERED_DCAE_DIFFUSION_MODEL.keys())}")
