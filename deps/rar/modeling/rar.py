...
from deps.rar.modeling.modules import BaseModel
...
class RAR(BaseModel):
    ...
    def set_kv_cache_flag(self, flag: bool):
        for block in self.blocks:
            block.attn.kv_cache = flag
    ...
    def generate(self,
        ...
        if ids is None:
            ...
            start_token = ids.shape[1]

        return_prob = kwargs.get("return_prob", False)

        gen = kwargs.get("torch_generator", None)
        ...
        cfg_orders = None
        probs = None
        seq_len = kwargs.get("seq_len", self.image_seq_len)

        for step in range(start_token, seq_len):
            ...
            if logit_processor is not None:
                logits = logit_processor(past_ids=ids, logits=logits)

            if randomize_temperature > 0:
