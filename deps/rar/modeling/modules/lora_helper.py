...
def apply_lora(model, config, check=False):
    """Applies LoRA to the model."""
    
    lora_target_modules: tuple = ("attn.qkv", "attn.proj", "mlp.fc1", "mlp.fc2")
    apply_lora_to_linear(model, config.training.lora_r, config.training.lora_alpha, config.training.lora_dropout, lora_target_modules)
    mark_only_lora_as_trainable(model)
