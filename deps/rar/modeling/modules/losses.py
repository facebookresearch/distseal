...
# Compatibility wrapper for autocast decorator
def autocast_decorator(enabled=True, dtype=None):
    def decorator(func):
...
class DistillARLoss(nn.Module):
    def __init__(self, config):
        super().__init__()
        self.ar_loss = ARLoss(config)
        self.distill_loss = torch.nn.KLDivLoss(reduction="batchmean")
        self.distill_w = config.training.get("distill_weight", 0.0)
    
    def distill_forward(self, logits: torch.Tensor, ref_logits: torch.Tensor) -> torch.Tensor:
        shift_logits = (logits[..., :-1, :]).log_softmax(dim=-1)
        shift_ref_logits = (ref_logits[..., :-1, :]).softmax(dim=-1)
        shift_logits = shift_logits.view(-1, shift_logits.size(-1))
        shift_ref_logits = shift_ref_logits.view(-1, shift_ref_logits.size(-1))
        distill_loss = self.distill_loss(shift_logits, shift_ref_logits)
        return distill_loss
    
    def forward(self, logits: torch.Tensor, labels: torch.Tensor, ref_logits: torch.Tensor) -> Tuple[torch.Tensor, Mapping[Text, torch.Tensor]]:
        ar_loss, d = self.ar_loss(logits, labels)
        distill_loss = self.distill_forward(logits, ref_logits)
        total_loss = ar_loss + distill_loss * self.distill_w
        d["distill_loss"] = distill_loss.detach()
        d["ar_loss"] = d.pop("loss")
        d["loss"] = total_loss.detach()
