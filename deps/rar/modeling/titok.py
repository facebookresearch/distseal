...
from deps.rar.modeling.modules.blocks import TiTokEncoder, TiTokDecoder
from deps.rar.modeling.quantizer.quantizer import VectorQuantizer, DiagonalGaussianDistribution
...
try:
    ...
    # Fallback for incompatible huggingface_hub versions
    print(f"Warning: Could not import PyTorchModelHubMixin: {e}")
...
class PretrainedTokenizer(nn.Module):
    def __init__(self, pretrained_weight):
        ...
        # self.quantize = VectorQuantizer2(n_e=1024, e_dim=256, beta=0.25, norm_embed=True)
        # Load pretrained weights
        state_dict = torch.load(pretrained_weight, map_location=torch.device("cpu"))
        if "state_dict" in state_dict:
            state_dict = state_dict["state_dict"]
        # Rename keys containing 'quantizer' to 'quantize'
        state_dict = {k.replace('quantizer', 'quantize'): v for k, v in state_dict.items()}
