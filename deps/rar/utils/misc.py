...
def tensor_to_message(tensor: torch.Tensor) -> str:
    ...
    return " ".join(map(str, tensor.int().tolist()))
...
def convert_json_to_jsonl(input_pattern, output_file):
    with open(output_file, 'w', encoding='utf-8') as outfile:
        for filename in tqdm(glob(input_pattern)):
...
def message_accuracy(message: torch.Tensor, gt_message: torch.Tensor) -> Dict[str, torch.Tensor]:
    ...
    # p-value: probability of observing a bit accuracy as high as the one observed, assuming the null hypothesis that 
    ...
    for success_k, bit_acc, word_acc in zip(successes_k, bit_accs, word_accs):
        ...
        all_stats["word_acc"].append(int(word_acc.item()))
        all_stats["p_value"].append(pval.pvalue)
        all_stats["log10_p_value"].append(np.log10(pval.pvalue) if pval.pvalue > 0 else -100)

    return {
        "bit_acc": all_stats["bit_acc"],
        "word_acc": all_stats["word_acc"],
        "p_value": all_stats["p_value"],
        "log10_p_value": all_stats["log10_p_value"],
    }
