...
from rar.data.local_reader import PretokenizedDataSetJSONL

from rar.modeling.modules.ema_model import EMAModel
from rar.modeling.modules.losses import ReconstructionLoss_Stage1, ReconstructionLoss_Stage2, ReconstructionLoss_Single_Stage, MLMLoss, ARLoss, DistillARLoss
...
from rar.modeling.modules.lora_helper import apply_lora
...
from distseal.utils.cfg import setup_model_from_checkpoint, message_to_tensor
...
def create_pretrained_tokenizer(config, accelerator=None):
    ...
    if not config.model.vq_model.pretrained_tokenizer_weight:
        # Download pretrained tokenizer from huggingface.
        config.model.vq_model.pretrained_tokenizer_weight = resolve_checkpoint(
           "maskgit-vqgan-imagenet-f16-256.bin", hf_namespace= "fun-research/TiTok"
        )
...
def create_wm_model(config):
    if config.model.get("wm", None):
        wm_model = setup_model_from_checkpoint(config.model.wm.checkpoint_path)
        wm_model.img_size = 256  # set to 256 for latent watermarking
        wm_model.eval()
        wm_model.requires_grad_(False)
        
        msg_file = config.model.wm.wm_message if config.model.wm.get("wm_message", "") else None
...
def create_model_and_loss_module(config, logger, accelerator,
    ...
    if model_type == "titok":
        ...
        model_cls = RAR
        if config.training.get("distill_weight", 0.0) > 0.0:
            loss_cls = DistillARLoss
        else:
            loss_cls = ARLoss
    ...
    if config.training.get("use_lora", False):
        apply_lora(model, config, check=True)
    ...
    # In distillation setting, we need to create a frozen reference model
    ref_model = None
    if config.experiment.get("teacher_weight", ""):
        ref_model = model_cls(config)
        teacher_weight = torch.load(config.experiment.teacher_weight, map_location="cpu")
        msg = ref_model.load_state_dict(teacher_weight, strict=False)
        logger.info(f"loading teacher weight from {config.experiment.teacher_weight}, msg: {msg}")
        ref_model.eval()
    ...
    # freeze model for debugging
    if config.experiment.get("freeze_model", False):
        for param in model.parameters():
            param.requires_grad = False
        accelerator.print("Freeze the model for debugging.")

    return model, ema_model, ref_model, loss_module
...
def create_dataloader(config, logger, accelerator):
    ...
    # raise ValueError(f"Debug: {dataset_config.get('pretokenization', '')} and {dataset_config.get('dataset_with_text_label', False) }")
    assert dataset_config.get("pretokenization", ""), f"A pretokenized dataset is required for T2I training."
        
    train_dataloader = DataLoader(
        PretokenizedDataSetJSONL(dataset_config.pretokenization),
    ...
    accelerator.print(f"[Rank {accelerator.process_index}/{accelerator.num_processes}] Debug: num_batches = {train_dataloader.num_batches}, max_train_examples = {config.experiment.max_train_examples}, total_batch_size = {total_batch_size_without_accum}")
...
def auto_resume(config, logger, accelerator, ema_model,
    ...
    if config.experiment.resume:
        ...
        if len(local_ckpt_list) >= 1:
            ...
            if len(checkpoint_paths) > 1 and Path(checkpoint_paths[0]).exists():
    ...
    for i, batch in enumerate(train_dataloader):
        ...
        with accelerator.accumulate([model, loss_module]):
            ...
            if (
                ...
                # log_histogram(model, accelerator, global_step + 1)
...
def train_one_epoch_generator(
    ...
    for i, batch in enumerate(train_dataloader):
        ...
        if config.dataset.params.get("pretokenization", ""):
            ...
            # tokenize on the fly
            if "image" in batch:
                assert tokenizer is not None, "Need a tokenizer to get input tokens."
                images = batch["image"].to(
                    accelerator.device, memory_format=torch.contiguous_format, non_blocking=True
                )
        ...
        with accelerator.accumulate([model]):
            ...
            if model_type == "maskgit":
                ...
                if ref_model is not None:
                    with torch.no_grad():
                        ref_logits = ref_model(input_tokens, condition)
                    loss, loss_dict = loss_module(logits, labels, ref_logits=ref_logits)
                else:
            ...
            if any(p.requires_grad for p in model.parameters()):
                ...
                # Skip gradient computation when all layers are frozen
                pass
    ...
    accelerator.print(f"[{accelerator.process_index}] Finished epoch with local step = {i}, global step = {global_step}, read batches = {i * config.training.gradient_accumulation_steps * config.training.per_gpu_batch_size}")
...
def generate_images(model, tokenizer, accelerator, 
                    global_step, output_dir, logger, config=None, wm_model=None, wm_msg=None):
    model.eval()
    tokenizer.eval()
    ...
    images_for_saving = make_viz_from_samples_generation(
        generated_image)
    ...
    # Calculate bitaccuracy if tokenizer is available
    if wm_model is not None:
        generated = torch.clamp(generated_image, 0.0, 1.0) * 255.0
        generated_pils = [T.functional.to_pil_image(img.cpu().byte()) for img in generated]
        bit_accs = []
        for img in generated_pils:
            img_tensor_ = T.ToTensor()(img).unsqueeze(0).to(accelerator.device)
            logits = wm_model.detector(img_tensor_)[:, 1:]
            pred = (logits > 0).int()
            bitaccuracy = message_accuracy(pred, wm_msg)
            bit_accs.append(bitaccuracy)
        bitaccuracy = sum(bit_accs) / len(generated_pils)
        logger.info(f"Bit Accuracy at step {global_step}: {bitaccuracy:.4f}")
        accelerator.log({"train/bit_accuracy": bitaccuracy}, step=global_step)
...
def log_histogram(model, accelerator, global_step):
    import wandb

    for name, param in model.named_parameters():
        accelerator.log({"weights/" + name: wandb.Histogram(param.detach().cpu().numpy())}, step=global_step)
        if param.grad is not None:
            accelerator.log({"grads/" + name: wandb.Histogram(param.grad.detach().cpu().numpy())}, step=global_step)