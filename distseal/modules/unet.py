...
"""
Test with:
    python -m distseal.modules.unet
"""
...
class UNetMsg(nn.Module):
    def __init__(
        ...
        just_upsampling: bool = False, # if True, skip the downsampling path
        coverless: bool = False, # if True, do not use the input image, only the message
        *args, **kwargs
    ) -> None:
        ...
        self.just_upsampling = just_upsampling
        self.coverless = coverless
        ...
        for ii in reversed(range(len(z_channels_list) - 1)):
            if just_upsampling:
                self.ups.append(UBlock(
                    z_channels_list[ii + 1], z_channels_list[ii], act_layer, norm_layer, upsampling_type, id_init, conv_layer=conv_layer))
            else:
                self.ups.append(UBlock(
    ...
    def forward(
        ...
        # Initial convolution
        if self.coverless:
            imgs = torch.zeros(imgs.shape, device=imgs.device, dtype=imgs.dtype)
        x1 = self.inc(imgs)
        hiddens = [x1]

        # Downward path
        if not self.just_upsampling:
...
def test_script_unetmsg() -> bool:
    ...
    import torch
    from distseal.modules.msg_processor import MsgProcessor
