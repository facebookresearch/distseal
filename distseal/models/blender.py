...
class Blender(nn.Module):
    ...
    AVAILABLE_BLENDING_METHODS = [
        "additive", "multiplicative", "multiplicative_max", "multiplicative_mean", "spatial_smoothed", "variance_based", "l2_ball_blend", "norm_prop_blend"
    ]
    ...
    def __init__(self,  scaling_i, scaling_w, method="additive"):
        ...
        self.blend_methods = {
            ...
            "multiplicative_max": self.multiplicative_max_blend,
            "multiplicative_mean": self.multiplicative_mean_blend,
            "spatial_smoothed": self.spatial_smoothed_blend,
            "variance_based": self.variance_based_blend,
            "l2_ball_blend": self.l2_ball_blend,
            "norm_prop_blend": self.norm_prop_blend,
        }
    ...
    def l2_ball_blend(self, imgs, preds_w):
        """
        Adds the watermark to the original images.
        ...
        - Can allow the network to learn watermark strength by adjusting the scaling factors.
        """
        def project_to_l2_ball(x, eps):
            """
            Projects tensor x to the L2 ball of radius eps.
            """
            norm = x.norm(p=2, dim=tuple(range(1, x.dim())), keepdim=True)
            factor = torch.minimum(eps / norm, torch.ones_like(norm))
            return x * factor

        image_min = torch.amin(imgs, dim=tuple(range(1, imgs.dim())), keepdim=True)
        image_max = torch.amax(imgs, dim=tuple(range(1, imgs.dim())), keepdim=True)
        output = self.scaling_i * imgs + project_to_l2_ball(preds_w, self.scaling_w)
        return torch.clamp(output, image_min, image_max)
    
    def norm_prop_blend(self, imgs, preds_w):
        """
        Adds the watermark to the original images.
        ...
        - Can allow the network to learn watermark strength by adjusting the scaling factors.
        """
        def project_to_l2_ball(x, reference, eps):
            """
            Projects tensor x to the L2 ball of radius eps * norm(reference).
            """
            norm = x.norm(p=2, dim=1, keepdim=True)
            norm_reference = reference.norm(p=2, dim=1, keepdim=True)
            factor = torch.minimum(eps * norm_reference / norm, torch.ones_like(norm))
            return x * factor

        image_min = torch.amin(imgs, dim=tuple(range(1, imgs.dim())), keepdim=True)
        image_max = torch.amax(imgs, dim=tuple(range(1, imgs.dim())), keepdim=True)
        output = self.scaling_i * imgs + project_to_l2_ball(preds_w, imgs, self.scaling_w)
        return torch.clamp(output, image_min, image_max)
    ...
    def multiplicative_max_blend(self, imgs, preds_w):
        return imgs + self.scaling_w * preds_w * torch.amax(torch.abs(imgs), dim=(1, 2, 3), keepdim=True)

    def multiplicative_mean_blend(self, imgs, preds_w):
        return imgs + self.scaling_w * preds_w * torch.mean(torch.abs(imgs), dim=(1, 2, 3), keepdim=True)
