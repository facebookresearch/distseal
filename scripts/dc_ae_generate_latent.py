...
from deps.efficientvit.ae_model_zoo import DCAE_HF, REGISTERED_DCAE_MODEL, AutoencoderKL
from deps.efficientvit.apps.data_provider.sampler import DistributedRangedSampler
...
from deps.efficientvit.apps.utils.image import CustomImageFolder, DMCrop
from deps.efficientvit.models.utils.network import get_dtype_from_str
...
class GenerateLatentConfig:
    ...
    watermarker_path: Optional[str] = None
    watermarker_scaling_w: float = 0.1
    watermarker_seed: int = 42
    watermarker_latent: bool = True
...
def main():
    ...
    if cfg.watermarker_path:
        watermarker = setup_model_from_checkpoint(cfg.watermarker_path)
        watermarker.blender.scaling_w = cfg.watermarker_scaling_w
        # watermarker.img_size = 512
        ...
        msg = torch.randint(0, 2, (1, msg.shape[1]), device=device, generator=g)
    ...
    if is_master():
        ...
        # Save the watermark message (msg) to cfg.latent_root_path
        if cfg.watermarker_path and is_master():
            msg_path = os.path.join(cfg.latent_root_path, "watermark_msg.npy")
            np.save(msg_path, msg.cpu().numpy())
    dist_barrier()
    ...
    for batch_idx, input_dict in tqdm(enumerate(data_loader), total=len(data_loader), disable=not is_master()):
        ...
        if cfg.watermarker_path and not cfg.watermarker_latent:
            msg_batch = msg.repeat(images.shape[0], 1).cuda()
            images = (images * 0.5 + 0.5).clamp(0, 1)  # to [0,1]
            if watermarker.embedder.yuv:  # take y channel only
                preds_w = watermarker.embedder(watermarker.rgb2yuv(images)[:, 0:1], msg_batch)
            else:
                preds_w = watermarker.embedder(images, msg_batch)
            images = watermarker.blender(images, preds_w).clamp(0, 1)
            images = images * 2 - 1  # to [-1,1]
        latents = model.encode(images)
        if cfg.watermarker_path and cfg.watermarker_latent:
            msg_batch = msg.repeat(images.shape[0], 1).cuda()
            preds_w = watermarker.embedder(latents, msg_batch)
            latents = watermarker.blender(latents, preds_w)
